...
bool Proxy::Attach(HMODULE CurrentModule)
{
	...
	// Initializes the filesystem and logger.
	static FS Filesystem(ProxyName.substr(0, ProxyName.find_last_of("\\") + 1));
	Global::Filesystem = &Filesystem;

	if (Filesystem.CreateSubfolder("Cydeloader"))
	{
		Filesystem.Create_File("Config.ini", Filesystem.GetRelativePath() + "Cydeloader\\");
	}
